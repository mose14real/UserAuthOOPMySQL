...
## MAIN FOLDER: UserAuthOOPMYSQL
  ...
  ### `action.php`
    this initializes all the classes for you, note that none of this is functional yet and you will need to implement all the classes initialised for this to function properly
...
- ## `database`
  You are going to work on the database you had in the previous task called zuriphp which contains the following columns
   ...
   - `Gender`
   - `password`
   - import the sql file called users.sql to have some initial users in your database for testin `this is in case you have not done that before`

- ## Task `1` Database Connection
  Inside the classes folder in the `Dbh.php` file, 
  - create a class called `Dbh` with the following properties
    - `hostname`
    - `username`
    - `password`
    - `dbname`
...
- ## Task `2` UserAuthentication Logic Class
  ...
  - Create a classes called UserAuth which has the following
    ...
    - Has the following methods
      ...
      - `updateUserPassword($email, $password)` Updates User Password (This will first called the checkEmailExist method). if useremailExist then update the password and redirect to the login page, else redirect to password reset page
...
- ## Task `3` Router Logic File
  Inside the classes folder of the `Route.php` file
  ...
  Take Your time, read the task over and over till you understand it before attempting, you are free to create meets to discus this.
